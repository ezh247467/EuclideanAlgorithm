...
def gcdWithRecursion(int1, int2):
    if int2 == 0:
        ...
        remainder = int1 % int2
        typeOutLine(f"{int1} = {int2}({int1 // int2}) + {remainder}")
...
def linearCombination(int1, int2):
    ...
    while remainder > 0:
        ...
        remainder = int1 % int2
    if len(pastGCD) == 0:
        typeOutLine(f"{int2} = {int1}(0) + {int2}(1)")
        typeOutLine(f"For the linear combination of {int1}x + {int2}y, we have x = 0 and y = 1")
    else:
        iter = len(pastGCD) - 1
        ...
        tracker = 0
        while iter > -1:
            typeOutLine(f"{int2} = {leftNum}({leftMultiplier}) + {rightNum}({rightMultiplier})")
            iter -= 1
            if iter < 0:
                break
            if tracker % 2 == 0:
        ...
        if tempInt == rightNum:
            ...
            typeOutLine(f"{int2} = {leftNum}({leftMultiplier}) + {rightNum}({rightMultiplier})")
        typeOutLine(f"For the linear combination of {leftNum}x + {rightNum}y, we have x = {leftMultiplier} and y = {rightMultiplier}")
...
typeOutLine(f"The gcd of {num1} and {num2} is {gcdWithRecursion(num1, num2)}")
# Using a while loop to find GCD
typeOutLine("\nNow using a While Loop to find GCD...")
typeOutLine(f"The gcd of {num1} and {num2} is {gcdWithWhileLoop(num1, num2)}")
