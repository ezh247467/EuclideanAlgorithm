...
def gcdWithRecursion(int1, int2):
    if int2 == 0:
        ...
        remainder = int1 % int2
        typeOut(f"{int1} = {int2}({int1 // int2}) + {remainder}")
...
def linearCombination(int1, int2):
    ...
    while remainder > 0:
        quotient = int1 // int2
        pastGCD.append((int1, int2, quotient))
        int1, int2 = int2, remainder
        remainder = int1 % int2
    if len(pastGCD) == 0:
        typeOut(f"{int2} = {int1}(0) + {int2}(1)")
        typeOut(f"For the linear combination of {int1}x + {int2}y, we have x = 0 and y = 1")
    else:
        iter = len(pastGCD) - 1
...
typeOut(f"The gcd of {num1} and {num2} is {gcdWithWhileLoop(num1, num2)}")
# Turns out you need to use the while loop method to find the linear combination
